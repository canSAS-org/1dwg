...
# $Date: 2009-01-12 10:29:06 -0600 (Mon, 12 Jan 2009) $
# $Author: prjemian $
# $Revision: 67 $
# $HeadURL: http://svn.smallangles.net/svn/canSAS/1dwg/trunk/cansasxml-html.xsl $
# $Id: cansasxml-extended-html.xsl 67 2012-03-26 15:00:00Z smk $
########### SVN repository information ###################
...
<xsl:stylesheet version="1.0"
	...
	xmlns:tof-tx="urn:transmission:spectrum"
	>
...
<!-- DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd" -->
		<html>
			...
			<body>
				...
				<small>generated using <TT>cansasxml-extended-html.xsl</TT> from canSAS</small>
				<BR />
				<table border="2">
					<tr>
				...
				<xsl:apply-templates  />
				<hr />
				<small><center>$Id: cansasxml-extended-html.xsl 67 2012-03-26 15:00:00Z smk $</center></small>
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template match="cs:SASroot">
		<xsl:for-each select="cs:SASentry">
			...
			<table border="2">
				<tr>
					<th>SAS data</th>
					<xsl:if test="count(tof-tx:sample_transmission_spectrum)!=0"><th>Sample transmission</th></xsl:if>
					<xsl:if test="count(tof-tx:can_transmission_spectrum)!=0"><th>Can transmission</th></xsl:if>
					<th>Selected Metadata</th>
				</tr>
				<tr>
					<td valign="top"><xsl:apply-templates  select="cs:SASdata" /></td>
					<xsl:apply-templates  select="tof-tx:sample_transmission_spectrum" />
					<xsl:apply-templates  select="tof-tx:can_transmission_spectrum" />
					<td valign="top">
						<table border="2">
							<tr bgcolor="lavender">
								<th>name</th>
	...
	<xsl:template match="tof-tx:sample_transmission_spectrum">
		<td valign="top">
		    <table border="2">
			<caption><xsl:if 
				test="@name!=''"><xsl:value-of select="@name" /></xsl:if> (<xsl:value-of 
				select="count(tof-tx:data)" /> points)
				<a id="#TSdata-{generate-id(.)}"  name="TSdata-{generate-id(.)}" />
			</caption>
			<tr bgcolor="lavender">
				<xsl:for-each select="tof-tx:data[1]/*">
					<th>
						<xsl:value-of select="name()" /> 
				...
				<xsl:for-each select="tof-tx:data">
					<tr>
						<xsl:for-each select="*">
	...
	<xsl:template match="tof-tx:can_transmission_spectrum">
		<td valign="top">
			<table border="2">
			<caption><xsl:if 
				test="@name!=''"><xsl:value-of select="@name" /></xsl:if> (<xsl:value-of 
